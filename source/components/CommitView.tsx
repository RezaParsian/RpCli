...
export default function CommitView({ useAll, token, onInvalidToken }: Props) {
	...
	useEffect(() => {
		if (state !== 'success' && state !== 'error' && state !== 'no-diff' && state !== 'cancelled') return
	...
	if (state === 'review') {
		return (
			<Box flexDirection="column">
				...
				<Text>
					Do you want to commit with this message? <Text color="yellow">(y/n)</Text>
				</Text>
			</Box>
		)
	}
	...
	if (state === 'no-diff') {
		return (
			<Box flexDirection="column">
				...
				{!useAll && <Text color="yellow">{'Tip: Stage your changes first using `git add .` or `git add <file>`'}</Text>}
			</Box>
		)
	}
