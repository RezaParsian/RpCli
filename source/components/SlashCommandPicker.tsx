...
export default function SlashCommandPicker({ query, onCancel, onQueryChange, onSelect }: Props) {
	const matches = useMemo(() => {
		const normalizedQuery = query.toLowerCase()
		return slashCommands.filter((command) =>
			[command.name, ...(command.aliases ?? [])].some((name) => name.slice(1).includes(normalizedQuery))
		)
	}, [query])
	...
	useInput((input, key) => {
		...
		if (key.upArrow || key.downArrow) {
			...
			setSelectedIndex((current) => (matches.length === 0 ? 0 : (current + direction + matches.length) % matches.length))
			return
		}
