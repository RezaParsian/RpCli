...
function renderBlock(token: AnyToken, key: number, isThinking = false): React.ReactNode {
	...
	if (token.type === 'code') {
		return (
			<Box key={key} flexDirection="column" marginY={1}>
				<SyntaxHighlight code={token.text || ''} language={syntaxLanguage(token.lang)} />
			</Box>
		)
	}
	...
	if (token.type === 'table') {
		const headers = (token.header ?? []).map((cell) => (cell.tokens ? inlineText(cell.tokens) : cell.text ?? ''))

		const rows = (token.rows ?? []).map((row) => row.map((cell) => (cell.tokens ? inlineText(cell.tokens) : cell.text ?? '')))
